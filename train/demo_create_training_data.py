...
ti_env = lmdb.open('data/train_input', map_size=1024000000000) # 修改map_size，避免OverflowError
...
for dset in range(statlist.shape[0]):   # for python3
    ...
    with ti_env.begin(write=True) as ti_txn, tl_env.begin(write=True) as tl_txn:
        for i in range(split):
            ...
            ti_txn.put('{:08}'.format(tcount).encode(), d_input.SerializeToString())
            tl_txn.put('{:08}'.format(tcount).encode(), d_label.SerializeToString())
            tcount = tcount + 1

    with vi_env.begin(write=True) as vi_txn, vl_env.begin(write=True) as vl_txn:
        for i in range(split + 1, idx[0].shape[0]):
            r = idx[0][perms[i]]
            c = idx[1][perms[i]]
            patch = img_mean[r - r % 8:r - r % 8 + 8, c - c % 8:c - c % 8 + 8, :].reshape(192)
            color = img_mean[r, c, :].reshape(3)
